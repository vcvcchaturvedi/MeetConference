...
var conference = function(config) {
    ...
    function onDefaultSocketResponse(response) {
		...
		if(response.sharedScreen){
			config.onScreenShare(response.userToken,response.peerlist,response.tracks);
		}
        if (isGetNewRoom && response.roomToken && response.broadcaster) config.onRoomFound(response);

        if (response.newParticipant && self.joinedARoom && self.broadcasterid == response.userToken) onNewParticipant(response.newParticipant,response.userName,response.ishost);
